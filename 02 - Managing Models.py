...
# MAGIC %md
# MAGIC Remove any prior executions of this script.  Note that the folder is now `dbfs:/mlflow/taxi_total_amount_2` instead of `taxi_total_amount`.  This way, you will not overwrite executions from the prior lab.
...
# MAGIC %md
# MAGIC Train the model with what were the most successful hyperparameters in the prior lab.
...
# MAGIC %md
# MAGIC ## Managing a Model via Code
# MAGIC 
...
# MAGIC %md
# MAGIC Next, retrieve the latest run of model training.  This is located in a folder named by the run's unique identifier (`run_uuid`).  From there, you wrote the model to a `model` folder in `train_nyc_taxi()`.
...
# MAGIC %md
# MAGIC Once the model is available, you can update the currently registered model.  The following method calls update the model description and the model version's description, respectively.
# MAGIC 
...
# MAGIC %md
# MAGIC With this function in place, build a sample input and generate the forecast for the `Production` model.  Use the `testData` DataFrame that you created earlier in this lab, as it has all of the inputs in the right shape for performing inference.
...
# MAGIC %md
# MAGIC Now, retrieve the latest version of the `NYC Taxi Amount API` model.
...
# MAGIC %md
# MAGIC Use the model version description to explain how this model differs from the others.  In this case, you changed the value of the *max_iter* parameter from 50 to 500 and also changed the ElasticNet parameter.
...
# MAGIC %md
# MAGIC Before moving this model to production, you can stage the model by moving this version to `Staging`.
...
# MAGIC %md
# MAGIC The reason the `forecast_nyc_taxi_amount()` function included a model stage is to allow testing of the `Staging` model before transitioning it to `Production`.
# MAGIC 
...
# MAGIC %md
# MAGIC It looks like this didn't change the results very much, but there is a small difference.  Let's say that you are confident in the new model and are ready to make it the new production model.

# COMMAND ----------

# MAGIC %md
# MAGIC ### Transitioning a New Version of a Model
# MAGIC 
...
# MAGIC %md
# MAGIC Now both model versions are tagged as production.  Which one will Azure Databricks use?
...
# MAGIC %md
# MAGIC It turns out that Azure Databricks looks for the latest model version with a given tag.  We can tell because the predicted amount is the same amount we saw from the most recently trained model--in other words, the one we most recently promoted to `Production`.
# MAGIC 
...
# MAGIC %md
# MAGIC If you wish to go further and delete a model version, a method is available for that as well.
...
# MAGIC %md
# MAGIC Finally, you will be able to delete the registered model.
