...
# MAGIC %md
# MAGIC # Using MLflow to Track Experiments
# MAGIC 
...
# MAGIC %md
# MAGIC ### Featurize and Train the Model
# MAGIC 
...
#normalize the hour to between 0 and 2 * pi. Then convert into sine and cosine value
...
#remove rows that do not have a target
dataset = dataset.filter(dataset.totalAmount.isNotNull())

#convert the boolean into an integer 
...
# MAGIC %md
# MAGIC Trying the model with a different set of hyperparameters is as easy as changing the inputs to our method.
...
# MAGIC %md
# MAGIC In this case, the R^2 went up and RMSE went down, both of which are good signs.  This model is marginally better than the first:  we are approximately 15 cents more accurate and explain approximately 1% more of the total variance.  These are not enormous changes, but are a step in the right direction.
...
# MAGIC %md
# MAGIC This third execution is somewhere between the first and the second.  As we can see in the image below, this suffers from the same skew as our second model.
...
# MAGIC %md
# MAGIC This information is now stored in the MLflow tracking server.  In the next exercise, we will learn two methods of working with this data:  by reviewing the experiment details in the Databricks user interface, as well as interacting with MLflow programmatically.

# COMMAND ----------

# MAGIC %md
# MAGIC ## Reviewing Experiment Metrics
# MAGIC 
...
# MAGIC %md
# MAGIC Next, generate a list of experiments.
...
# MAGIC %md
# MAGIC The table above includes a variety of information about each run, including details on your logged metrics, the parameters you used, and a variety of system- and user-generated tags.  This makes it easy to export information on an experiment for further analysis.
# MAGIC 
